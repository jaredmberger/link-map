...
if(backup.format!=='link-map-kv-recovery')fail('Unexpected backup format.');
...
console.log('Link Map recovery backup is valid.');
console.log(`Exported: ${backup.exportedAt||'unknown'}`);
console.log(`KV keys: ${backup.data.entries.length}`);
console.log(`SHA-256: ${actual}`);
function fail(message){console.error(`Invalid Link Map recovery backup: ${message}`);process.exit(1);}
