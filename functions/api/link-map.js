...
const CACHE_KEY='link-map-v2';
const SEED_PATHS=['/','/ships/ships','/site-map','/explore','/sitemap.xml'];
...
async function crawl(){
  const discovered=new Set(SEED_PATHS.filter(p=>!p.endsWith('.xml')));
  ...
  async function worker(){
    while(true){
      ...
      let res;
      try{res=await fetch(url,{headers:{'user-agent':'OceanLinerCurator-LinkMap/2.0','accept':'text/html,application/xhtml+xml'}})}catch{continue}
      ...
      for(const raw of links){
        ...
        const u=new URL(target);const p=u.pathname;
  ...
  return {
    ...
    source:'live-crawl',
    coverage:{discoveredPaths:discovered.size,crawledPages:pages.length,maxPages:MAX_PAGES}
  };
}
...
async function archivePaths(path){
  const out=new Set();
  try{
    const res=await fetch(new URL(path,SITE),{headers:{'user-agent':'OceanLinerCurator-LinkMap/2.0','accept':'text/html'}});
    if(!res.ok)return [];
    const html=await res.text();
...
async function sitemapPaths(){
  ...
  for(const candidate of ['/sitemap.xml','/sitemap_index.xml']){
    try{
      const res=await fetch(new URL(candidate,SITE),{headers:{'user-agent':'OceanLinerCurator-LinkMap/2.0'}});if(!res.ok)continue;
      const xml=await res.text();
      const locs=[...xml.matchAll(/<loc>\s*([^<]+?)\s*<\/loc>/gi)].map(m=>decode(m[1].trim()));
      for(const loc of locs){
        if(/\.xml(?:$|\?)/i.test(loc)){
          try{
            const child=await fetch(loc,{headers:{'user-agent':'OceanLinerCurator-LinkMap/2.0'}});if(!child.ok)continue;
            const childXml=await child.text();
            for(const m of childXml.matchAll(/<loc>\s*([^<]+?)\s*<\/loc>/gi)){
              const n=normalize(decode(m[1].trim()));if(n)out.add(new URL(n).pathname);
            }
          }catch{}
        }else{
